// src/pages/ForgotPasswordPage.js — Token-based reset, no OTP email needed
...
export default function ForgotPasswordPage() {
  ...
  const [resetData, setResetData]   = useState(null); // {reset_token, user_id}
  ...
  const checkEmail = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        return toast.error(data.message || 'Error occurred');
      }
  ...
  const resetPassword = async (e) => {
    ...
    const pwdOk = /^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[!@#$%^&*])/.test(newPassword);
    if (!pwdOk) return toast.error('Need uppercase, lowercase, number & special char');
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl">
          ...
          {step === 3 && (
            <div className="text-center">
              ...
              <p className="text-slate-300 mb-2">Password changed successfully!</p>
